...
import type { PostAttribute } from '../../../types/AppiService'

const props = defineProps<{ postId: string }>()

const attributes = ref<PostAttribute[]>([])
...
async function fetchAttributes() {
  loading.value = true
  try {
    const { data } = await AppiService.get<PostAttribute[]>(`/admin/content/posts/${props.postId}/attributes`)
...
async function createAttribute() {
  ...
  error.value = null
  try {
    const { data } = await AppiService.post<PostAttribute>(`/admin/content/posts/${props.postId}/attributes`, {
...
async function deleteAttribute(id: string) {
  ...
  error.value = null
  try {
    await AppiService.delete(`/admin/content/posts/${props.postId}/attributes/${id}`)
...
<template>
  <div class="flex flex-col gap-3">
    ...
    <div class="flex flex-col gap-2 p-3 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
      ...
      <DuiInput v-model="newAttr.name" block placeholder="Nombre" />
      <DuiInput v-model="newAttr.value" block placeholder="Valor" />
      <DuiButton
        type="button"
        color="primary"
        size="sm"
